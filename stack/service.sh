...
function build() {
	...
	if [[ $size > 0 ]] ; then
		for item in ${args[*]}; do
			docker build --rm -f ./config/$item/Dockerfile -t ghandalf/$item:${CONTAINERS_VERSION} -t ghandalf/$item:latest .
		done
	else
...
function clean() {
	...
	echo -e "${dt}${Red} Removing containers with tag none `docker rmi $(docker images -f 'dangling=true' -q)`${Color_Off}";
